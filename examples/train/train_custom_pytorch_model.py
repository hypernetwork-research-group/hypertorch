...
class CustomPyTorchHlpPredictor(nn.Module):
    """A hyperlink predictor implemented as a PyTorch model."""

    def __init__(self, in_channels: int = 2, hidden_channels: int = 8) -> None:
        super().__init__()
...
class LightningHlpPredictor(L.LightningModule):
    """Wrap a PyTorch hyperlink predictor for use with ``MultiModelTrainer``."""

    def __init__(
        self,
        model: CustomPyTorchHlpPredictor,
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Forward inputs to the wrapped PyTorch model.
    ...
    def configure_optimizers(self) -> optim.Adam:
        ...
        Returns:
            optimizer: Adam optimizer for the wrapped model parameters.
        """
...
if __name__ == "__main__":
    ...
    model = CustomPyTorchHlpPredictor(in_channels=2, hidden_channels=8)
    lightning_model = LightningHlpPredictor(model=model, metrics=metrics)
    ...
    with MultiModelTrainer(
        ...
        experiment_name="custom_pytorch_model",
        max_epochs=20,
        accelerator="auto",
        devices=1,
        log_every_n_steps=1,
