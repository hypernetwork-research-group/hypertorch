...
def test_model_hgnn_batch(tmp_path, sampling_strategy, batch, batch_size, request):
    ...
    multi_model_trainer(configs, path=tmp_path, experiment_name=f"hgnn_integration_test_{test_id}")
    ...
    assert (tmp_path / f"hgnn_integration_test_{test_id}" / "comparison" / "results.md").exists()
...
def test_model_hgnn_full(tmp_path, sampling_strategy, request):
    test_id = request.node.callspec.id
    num_features = NUM_FEATURES
    metrics = common_metrics()
