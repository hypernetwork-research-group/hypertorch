...
def test_model_node2vecgcn_batch(tmp_path, sampling_strategy, batch, batch_size, request):
...
def test_model_node2vecgcn_full(tmp_path, sampling_strategy, request):
    test_id = request.node.callspec.id
    num_features = NUM_FEATURES
    metrics = common_metrics()
    ...
    multi_model_trainer(
        configs, path=tmp_path, experiment_name=f"node2vecgcn_integration_test_{test_id}"
    )
