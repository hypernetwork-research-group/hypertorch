...
def test_model_node2vecslp(tmp_path, sampling_strategy):
    ...
    multi_model_trainer(
        configs=configs, path=tmp_path, experiment_name="node2vecslp_integration_test"
    )

    assert (tmp_path / "node2vecslp_integration_test" / "comparison" / "overall.tex").exists()
    assert (tmp_path / "node2vecslp_integration_test" / "comparison" / "test.tex").exists()
    assert (tmp_path / "node2vecslp_integration_test" / "comparison" / "results.md").exists()
