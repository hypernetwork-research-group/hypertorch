...
#### Why we limit the number of nodes and hyperedges in the tests to 5000? ####
#
#
...
# reasonable amount of time, we limit the number of nodes and hyperedges to 5000
...
def test_lpe_node_enricher(dataset_name):
    ...
    if dataset.hdata.num_nodes > 5000 or dataset.hdata.num_hyperedges > 5000:
        pass
...
if __name__ == "__main__":
    dataset = get_dataset_by_name("patent")
    print(f"Original dataset: {dataset}")
    _, _, to_enrich_dataset = split_dataset(dataset=dataset, node_space_setting="inductive")
    print(f"Dataset to enrich: {to_enrich_dataset}")
    to_enrich_dataset.enrich_node_features(
        enricher=LaplacianPositionalEncodingEnricher(
            num_features=NUM_FEATURES,
            num_nodes=to_enrich_dataset.hdata.num_nodes,
