...
from hypertorch.types import EdgeIndex, Graph
...
def test_graph_stores_edge_weights():
...
def test_remove_selfloops_keeps_edge_weights_aligned():
    graph = Graph(
        [[0, 1], [1, 1], [2, 3], [3, 3]],
        edge_weights=[0.25, 0.5, 0.75, 1.0],
    )
...
def test_smoothing_with_laplacian_is_equal_for_zero_and_no_drop_rate():
    ...
    smoothed_x_zero_dropout = Graph.smoothing_with_laplacian_matrix(x, gcn_laplacian, drop_rate=0.0)
...
def test_smoothing_with_laplacian_nonzero_drop_rate_changes_output():
    ...
    smoothed_x_with_dropout = Graph.smoothing_with_laplacian_matrix(
...
def test_smoothing_with_laplacian_drop_rate_stochastic():
    ...
    smoothed_x1 = Graph.smoothing_with_laplacian_matrix(x, gcn_laplacian.clone(), drop_rate=0.5)

    torch.manual_seed(99)
    smoothed_x2 = Graph.smoothing_with_laplacian_matrix(x, gcn_laplacian.clone(), drop_rate=0.5)
...
def test_edge_index_stores_edge_weights():
    ...
    assert torch.equal(edge_index.edge_weights, edge_weights)
...
def test_get_sparse_normalized_laplacian_uses_edge_weights():
    ...
    expected_gcn_laplacian = torch.tensor(
        [
            [1 / 3, 2 / 3],
            [2 / 3, 1 / 3],
        ],
    ...
    assert torch.allclose(gcn_laplacian.to_dense(), expected_gcn_laplacian, atol=1e-6)
...
def test_get_sparse_normalized_laplacian_does_not_double_count_existing_reverse_edges():
    ...
    expected_gcn_laplacian = torch.tensor(
        [
            [1 / 3, 2 / 3],  # If we double-counted the reverse edge, this would be [1/5, 4/5]
            [2 / 3, 1 / 3],  # If we double-counted the reverse edge, this would be [4/5, 1/5]
        ],
        dtype=torch.float,
    )
    assert torch.allclose(gcn_laplacian.to_dense(), expected_gcn_laplacian, atol=1e-6)
