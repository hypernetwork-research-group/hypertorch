...
from hypertorch.tests import MOCK_BASE_PATH
...
def test_validate_hif_data_returns_true_with_a_valid_schema():
    ...
    with patch("hypertorch.utils.hif_utils.__load_hif_schema", return_value=schema):
        assert validate_hif_data({"name": "hypertorch"})
...
def test_validate_hif_data_returns_true_with_valid_hif_data(hif_data):
    with patch("hypertorch.utils.hif_utils.__load_hif_schema") as mock_load_schema:
...
@pytest.mark.parametrize(
    "hif_data",
    [
        {"name": "hypertorch", "extra": "not allowed"},
        {"name": 123},
        {},
    ],
)
...
def test_validate_hif_json_opens_the_given_path(tmp_path):
    ...
    with (
        patch("hypertorch.utils.hif_utils.open", mock_open(read_data="{}")) as mock_file,
        patch("hypertorch.utils.hif_utils.validate_hif_data", return_value=True),
    ):
...
def test_validate_hif_json_with_url_success(tmp_path):
    ...
    with (
        patch("hypertorch.utils.hif_utils.get_cache_dir", return_value=str(tmp_path)),
        patch("hypertorch.utils.hif_utils.requests.get") as mock_get,
        patch("hypertorch.utils.hif_utils.resources.files", return_value=mock_files),
    ):
...
def test_validate_hif_json_with_url_timeout_fallback():
    ...
    with (
        patch("hypertorch.utils.hif_utils.requests.get") as mock_get,
        patch(
            "hypertorch.utils.hif_utils.resources.files", return_value=mock_files
        ) as mock_files_call,
    ):
        mock_get.side_effect = requests.Timeout("Connection timeout")
        validate_hif_json(path_valid)

        mock_files_call.assert_called_once_with("hypertorch.utils.schema")
...
def test_validate_hif_data_raises_when_schema_load_fails():
    with (
        patch(
            "hypertorch.utils.hif_utils.resources.files",
            side_effect=RuntimeError("no local schema"),
        ),
        patch(
            "hypertorch.utils.hif_utils.requests.get",
...
def test_load_hif_schema_loads_from_cache(tmp_path):
    ...
    with patch("hypertorch.utils.hif_utils.get_cache_dir", return_value=str(tmp_path)):
...
def test_get_datasets_shas_returns_shas_and_none_on_failure():
    ...
    with (
        patch("hypertorch.utils.hif_utils.HfApi") as mock_hf_api,
...
def test_get_dataset_sha_returns_sha():
    with patch("hypertorch.utils.hif_utils.HfApi") as mock_hf_api:
        info = MagicMock()
        info.sha = "sha-algebra"
...
def test_validate_hif_json_exception_raises_with_invalid_json(tmp_path):
    ...
    with patch("hypertorch.utils.hif_utils.requests.get") as mock_get:
        mock_get.side_effect = requests.Timeout("Connection timeout")
