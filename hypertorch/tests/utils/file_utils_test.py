...
def test_compress_json_bytes_as_zst_raises_on_compression_error():
    with (
        patch(
            "hypertorch.utils.file_utils.zstd.ZstdCompressor.compress",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (b'{"name": "hypertorch", "count": 1}', {"name": "hypertorch", "count": 1}),
        (b"{}", {}),
        (
            b'{"nodes": [1, 2], "ok": true, "hyperedges": []}',
...
def test_read_json_file_returns_parsed_data(tmp_path):
    ...
    json_path.write_text('{"name": "hypertorch"}', encoding="utf-8")

    result = from_file_to_json(str(json_path))

    assert result == {"name": "hypertorch"}
...
def test_from_zst_bytes_to_json_returns_parsed_data():
    payload = {"name": "hypertorch", "items": [1, 2]}
...
def test_write_dataset_to_disk_as_zst_uses_default_output_dir(tmp_path):
    with patch(
        "hypertorch.utils.file_utils.get_cache_dir", return_value=tmp_path / ".hypertorch_cache"
    ):
        write_dataset_to_disk_as_zst("algebra_tmp", b"content")

    assert (
        tmp_path / ".hypertorch_cache" / "datasets" / "algebra_tmp.json.zst"
    ).read_bytes() == b"content"


def test_write_dataset_to_disk_as_zst_raises_when_path_cannot_be_determined():
    with (
        patch("hypertorch.utils.file_utils.get_cache_dir", side_effect=OSError("boom")),
...
def test_find_project_root_handles_current_working_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hypertorch.utils.file_utils.Path.cwd", lambda: source_file)
...
def test_find_project_root_returns_current_directory_when_no_marker_exists(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hypertorch.utils.file_utils.Path.cwd", lambda: current_dir)
...
def test_get_cache_dir_uses_project_root_and_creates_cache_directory(tmp_path, monkeypatch):
    ...
    assert result == project_root / ".hypertorch_cache"
...
def test_get_cache_dir_uses_relative_override_from_cwd(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HYPERTORCH_CACHE_DIR", "cache/subdir")
...
def test_get_cache_dir_uses_absolute_override(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(current_dir)
    monkeypatch.setenv("HYPERTORCH_CACHE_DIR", str(absolute_cache_dir))
...
def test_get_cache_dir_can_skip_creation(tmp_path, monkeypatch):
    ...
    with patch("hypertorch.utils.file_utils.Path.mkdir") as mock_mkdir:
