...
def mock_cache_root(tmp_path):
    with patch("hypertorch.data.hif.get_cache_dir", return_value=str(tmp_path)):
        yield
...
def test_load_from_path_zst_uses_decompress(tmp_path, mock_hypergraph):
    ...
    with (
        patch(
            "hypertorch.data.hif.from_zst_file_to_json", return_value=payload
...
def test_load_from_path_raises_for_non_hif_compliant_json(tmp_path, mock_hypergraph):
    ...
    with (
        patch("hypertorch.data.hif.validate_hif_data", return_value=False),
...
def test_load_from_url_processes_zst_and_saves_to_disk(tmp_path, mock_hypergraph):
    ...
    with (
        patch("hypertorch.data.hif.requests.get") as mock_get,
        patch(
            "hypertorch.data.hif.from_zst_bytes_to_json", return_value=payload
        ) as mock_from_zst_bytes_to_json,
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch("hypertorch.data.hif.write_dataset_to_disk_as_zst") as mock_write_to_disk,
    ):
        mock_response = mock_get.return_value
        mock_response.status_code = 200
...
def test_load_from_url_processes_json_and_saves_compressed_copy(tmp_path, mock_hypergraph):
    ...
    with (
        patch("hypertorch.data.hif.requests.get") as mock_get,
        patch(
            "hypertorch.data.hif.from_bytes_to_json", return_value=payload
        ) as mock_read_json_bytes,
        patch(
            "hypertorch.data.hif.compress_json_bytes_as_zst", return_value=b"compressed"
        ) as mock_compress,
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch("hypertorch.data.hif.write_dataset_to_disk_as_zst") as mock_write_to_disk,
    ):
...
def test_load_from_url_processes_json_without_saving_to_disk(tmp_path, mock_hypergraph):
    ...
    with (
        ...
        patch("hypertorch.data.hif.compress_json_bytes_as_zst") as mock_compress,
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch("hypertorch.data.hif.write_dataset_to_disk_as_zst") as mock_write_to_disk,
    ):
        mock_response = mock_get.return_value
...
def test_load_skips_download_when_file_exists(tmp_path, mock_hypergraph):
    ...
    with (
        patch("hypertorch.data.hif.os.path.exists", return_value=True),
        patch("hypertorch.data.hif.requests.get") as mock_get,
        patch(
            "hypertorch.data.hif.from_zst_file_to_json", return_value=payload
        ) as mock_from_zst_file_to_json,
...
def test_hifloader_falls_back_to_hf_hub_download_when_github_raw_download_fails(
    ...
    with (
        ...
        patch("hypertorch.data.hif.requests.get") as mock_get,
        patch("hypertorch.data.hif.hf_hub_download", return_value=str(fallback_file)) as _,
        patch("hypertorch.data.hif.from_zst_file_to_json", return_value=payload),
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
...
def test_load_saves_downloaded_dataset_on_disk(tmp_path, mock_hypergraph):
    ...
    with (
        patch("hypertorch.data.hif.os.path.exists", return_value=False),
        patch("hypertorch.data.hif.requests.get") as mock_get,
        patch("hypertorch.data.hif.from_zst_bytes_to_json", return_value=payload),
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch("hypertorch.data.hif.os.path.abspath", return_value=str(tmp_path / "hif.py")),
        patch("hypertorch.data.hif.write_zst_file_to_disk") as mock_save_zst_file,
    ):
        mock_response = mock_get.return_value
        mock_response.status_code = 200
...
def test_load_by_name_uses_hf_revision_when_github_download_fails(tmp_path, mock_hypergraph):
    ...
    with (
        patch("hypertorch.data.hif.os.path.exists", return_value=False),
        patch("hypertorch.data.hif.os.path.isdir", return_value=False),
        patch("hypertorch.data.hif.requests.get", return_value=response),
        patch(
            "hypertorch.data.hif.hf_hub_download", return_value=str(fallback_file)
...
def test_load_by_name_cleans_hf_cache_and_locks(tmp_path, mock_hypergraph):
    ...
    with (
        patch("hypertorch.data.hif.os.path.exists", return_value=False),
        patch("hypertorch.data.hif.os.path.isdir", return_value=True),
        patch("hypertorch.data.hif.requests.get", return_value=response),
        patch("hypertorch.data.hif.hf_hub_download", return_value=str(fallback_file)),
        patch("hypertorch.data.hif.from_zst_file_to_json", return_value=payload),
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch("hypertorch.data.hif.shutil.rmtree") as mock_rmtree,
        pytest.warns(UserWarning, match="GitHub raw download failed"),
...
def test_load_by_name_raises_when_hf_sha_is_missing_on_fallback():
    ...
    with (
        patch("hypertorch.data.hif.os.path.exists", return_value=False),
        patch("hypertorch.data.hif.requests.get", return_value=response),
        patch("hypertorch.data.hif.hf_hub_download") as mock_hf_hub_download,
        pytest.warns(UserWarning, match="GitHub raw download failed"),
        pytest.raises(
            ValueError,
...
def test_load_by_name_raises_warn_when_fail_to_cleanup_hf_cache(tmp_path, mock_hypergraph):
    ...
    with (
        ...
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch(
            "hypertorch.data.hif.shutil.rmtree",
...
def test_load_by_name_raises_when_downloaded_hf_file_cannot_be_read(tmp_path):
    ...
    with (
        ...
        patch(
            "hypertorch.data.hif.from_zst_file_to_json",
...
def test_load_by_name_raises_when_downloaded_hf_content_cannot_be_written(tmp_path):
    ...
    with (
        ...
        patch("hypertorch.data.hif.requests.get", return_value=response),
        patch(
            "hypertorch.data.hif.hf_hub_download", return_value=str(tmp_path / "fallback.json.zst")
        ),
        patch("hypertorch.data.hif.from_zst_file_to_json", return_value=payload),
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch("hypertorch.data.hif.shutil.copyfile", side_effect=OSError("disk full")),
        pytest.warns(UserWarning, match="GitHub raw download failed"),
        pytest.raises(
            ValueError,
            match=re.compile(
...
def test_load_by_name_raises_when_saving_downloaded_dataset_fails(tmp_path):
    ...
    with (
        ...
        patch("hypertorch.data.hif.requests.get", return_value=response),
        patch(
            "hypertorch.data.hif.from_zst_bytes_to_json",
        ...
        patch("hypertorch.data.hif.validate_hif_data", return_value=True),
        patch(
            "hypertorch.data.hif.write_zst_file_to_disk",
...
def test_hifloader_download_failure_when_hf_token_is_invalid(tmp_path):
    ...
    with (
        ...
        patch("hypertorch.data.hif.os.getenv", return_value="invalid_token"),
    ):
        HIFLoader.load_by_name("algebra", hf_sha=hf_sha)
