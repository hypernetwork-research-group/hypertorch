...
def test_model_common_neighbors_nc(tmp_path, sampling_strategy, full, batch_size, request):
    ...
    train_test_loop(
        configs=configs,
        path=tmp_path,
        experiment_name=f"cn_nc_integration_test_{test_id}",
        train_loader=train_loader,
        val_loader=val_loader,
        test_loader=test_loader,
    )

    comparison_path = tmp_path / f"cn_nc_integration_test_{test_id}" / "comparison"
